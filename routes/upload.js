...
router.get('/materials/:classroomID', (req, res) => {
    StudyMaterial.find({ classroomID: req.params.classroomID }, (err, studyMaterials) => {
        if (err) {
...
router.get('/material/:id', (req, res) => {
    ...
    StudyMaterial.findById(req.params.id, (err, studyMaterial) => {
        if (err) {
            ...
            res.status(404).send({ message: 'error while getting single study material' })
        }
        res.download('uploads/materials/' + studyMaterial.fileName, 'sdf')
    })
})
